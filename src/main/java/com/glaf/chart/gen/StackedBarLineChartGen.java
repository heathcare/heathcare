...
public class StackedBarLineChartGen implements ChartGen {
	protected static final Log logger = LogFactory.getLog(StackedBarLineChartGen.class);

	public static void main(String[] paramArrayOfString) {
		StackedBarLineChartGen chartDemo = new StackedBarLineChartGen();
		...
		chartModel.setChartName("stackedbar_line");
		chartModel.setChartType("stackedbar_line");
	...
	public JFreeChart createChart(Chart chartModel) {
		logger.debug("------------StackedBarLineChartGen.createChart-----------------");
		...
		if (StringUtils.equals(chartModel.getEnable3DFlag(), "1")) {
			localJFreeChart = ChartFactory.createBarChart3D(chartModel.getChartTitle(), chartModel.getCoordinateX(),
					chartModel.getCoordinateY(), localDefaultCategoryDataset1, PlotOrientation.VERTICAL, true, true,
					false);
		} else {
		...
		if (StringUtils.equals(chartModel.getEnable3DFlag(), "1")) {
			...
			for (int i = 0; i <= 6; i++) {
				...
				lineRenderer.setSeriesShape(i, new Ellipse2D.Double(-2.0D, -2.0D, 4.0D, 4.0D));
				if (chartModel.getChartFont() != null && chartModel.getChartFontSize() > 0) {
					lineRenderer.setSeriesItemLabelFont(i,
							new Font(chartModel.getChartFont(), Font.PLAIN, chartModel.getChartFontSize()));
					lineRenderer.setSeriesItemLabelsVisible(i, true);
				} else {
					lineRenderer.setSeriesItemLabelFont(i, new Font("宋体", Font.PLAIN, 16));
					lineRenderer.setSeriesItemLabelsVisible(i, true);
				}
			}
