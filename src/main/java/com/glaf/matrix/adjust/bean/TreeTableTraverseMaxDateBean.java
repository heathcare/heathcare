...
public class TreeTableTraverseMaxDateBean {

	protected final Logger logger = LoggerFactory.getLogger(getClass());

	/**
	 * 递归每个节点及所有子孙节点，找出比adjustTime更大的值
	 * 
	 * @param component
	 * @param adjustTime
	 * @return
	 */
	public long getChildrenMaxValue(TreeComponent component, String index_id, long adjustTime) {
		long maxTime = adjustTime;
		List<TreeComponent> children = component.getComponents();
		if (children != null && children.size() > 0) {
			if (children.size() == 1) {
				maxTime = this.getChildrenMaxValue(children.get(0), index_id, maxTime);// 递归子节点
			} else {
				for (TreeComponent child : children) {
					if (!StringUtils.equals(index_id, child.getId())) {
						if (child.getLongValue() > 0) {
							if (child.getLongValue() > maxTime) {
								maxTime = child.getLongValue();// 取更新的时间
								logger.debug("取得更大的日期:" + child.getDataMap());
							}
						}
						maxTime = this.getChildrenMaxValue(child, index_id, maxTime);// 递归子节点
					}
				}
				int countChild = 0;
				for (TreeComponent child : children) {
	...
	/**
	 * 循环parent所有子节点，找出比adjustTime更大的值
	 * 
	 * @param parent
	 * @param adjustTime
	 * @return
	 */
	public long getParentMaxValue(TreeComponent parent, String index_id, long adjustTime) {
	 ...
	 * @return
	 */
	public long getParentMaxValue(TreeComponent parent, long adjustTime) {
		long maxTime = adjustTime;
		List<TreeComponent> children = parent.getComponents();
		if (children != null && children.size() > 0) {
			for (TreeComponent child : children) {
		...
		for (TreeComponent component : components) {
			if (StringUtils.equals(index_id, component.getId())) {
				...
				if (parent != null) {// 如果有父节点，继续处理父节点
					long maxTime = this.getParentMaxValue(parent, adjustTime);// 找同层节点中最小日期作为调整日期
					if (maxTime >= adjustTime) {
						adjustTime = maxTime;
					}
					long tmpTime = this.getParentMaxValue(parent, index_id, adjustTime);// 找出父节点的下级，即本节点同级的最小时间
	...
	public void populateParent(TreeDataAdjust dataAdjust, TreeComponent parent,
		...
		long maxTime = this.getChildrenMaxValue(parent, index_id, adjustTime);// 取得parent节点所有子节点最小的日期值
		logger.debug(" parent maxTime:" + DateUtils.getDate(DateUtils.toDate(maxTime)));
		if (maxTime > adjustTime) {// 如果其他节点的日期比调整日期小，保持不变
			return;
		}
		String dateColumn = dataAdjust.getAdjustColumn().toLowerCase();
		parent.setLongValue(adjustTime);
