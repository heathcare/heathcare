	...
	public void prepare(Tenant tenant, int year, int month, Map<String, Object> params) {
		...
		if (suitNo != 0) {
			DietaryTemplateService dietaryTemplateService = ContextFactory
					.getBean("com.glaf.heathcare.service.dietaryTemplateService");
			...
			if (tenantConfig != null) {
				...
				foodDRI = foodDRIService.getFoodDRIByAge(4);
				foodDRIPercent = foodDRIPercentService.getFoodDRIPercent("3-6", typeId);
			}
			...
			String sysFlag = ParamUtils.getString(params, "sysFlag");
			if (tenant.isSystemAdministrator()) {
				sysFlag = "Y";
			}

			DietaryTemplateQuery qx = new DietaryTemplateQuery();
			qx.sysFlag(sysFlag);
			qx.suitNo(suitNo);
			if (!"Y".equals(sysFlag)) {
				if (!tenant.isSystemAdministrator()) {
			...
			List<DietaryTemplate> templates = dietaryTemplateService.list(qx);
