...
public class MedicalSpotCheckPreprocessorV8 implements IReportPreprocessor {
    protected final static Log logger = LogFactory.getLog(MedicalSpotCheckPreprocessorV8.class);
    ...
    public void prepare(Tenant tenant, Map<String, Object> parameter) {
        logger.debug("--------------------MedicalSpotCheckPreprocessorV8--------");
        ...
        if (list != null && !list.isEmpty()) {
            ...
            String groupByField = ParamUtils.getString(parameter, "groupByField");

            Map<String, MedicalSpotCheckAggregationModel> totalMap = new TreeMap<String, MedicalSpotCheckAggregationModel>();
            JSONObject jsonObject = null;
            String groupBy = null;
            for (MedicalSpotCheck exam : list) {
                jsonObject = exam.toJsonObject();
                groupBy = jsonObject.getString(groupByField);
                if (StringUtils.isNotEmpty(groupBy)) {
                    MedicalSpotCheckAggregationModel model = totalMap.get(groupBy);
                    if (model == null) {
                        model = new MedicalSpotCheckAggregationModel();
                        model.setKey(groupBy);
                        totalMap.put(groupBy, model);
                    }
