...
public class MedicalSpotCheckPreprocessorV3 implements IReportPreprocessor {
	protected final static Log logger = LogFactory.getLog(MedicalSpotCheckPreprocessorV3.class);
	...
	public void prepare(Tenant tenant, Map<String, Object> parameter) {
		...
		if (list != null && !list.isEmpty()) {
			...
			Map<Integer, MedicalSpotCheckAggregationModel> totalMap = new TreeMap<Integer, MedicalSpotCheckAggregationModel>();
			for (MedicalSpotCheck exam : list) {
				MedicalSpotCheckAggregationModel model = totalMap.get(exam.getAgeOfTheMoon());
				if (model == null) {
				...
				this.populdate(exam, m1, "W/A");

				MedicalSpotCheckTotalModel m2 = model.getModel2();
				this.populdate(exam, m2, "H/A");
	...
	protected void populdate(MedicalSpotCheck exam, MedicalSpotCheckTotalModel model, String checkType) {
