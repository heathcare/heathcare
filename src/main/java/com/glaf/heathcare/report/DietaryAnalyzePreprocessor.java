			...
			if (rows != null && !rows.isEmpty()) {
				...
				if (totalPerson > 0 && days > 0) {
					...
					for (GoodsActualQuantity m : rows) {
						...
						fat = fat + food.getFat() * m.getQuantity() * 1000 / 100D;
						protein = protein + food.getProtein() * m.getQuantity() * 1000 / 100D;
						carbohydrate = carbohydrate + food.getCarbohydrate() * m.getQuantity() * 1000 / 100D;
						totalHeatEnergy = totalHeatEnergy + food.getHeatEnergy() * m.getQuantity() * 1000 / 100D;// 转成100克的热能

						total.setVitaminA(total.getVitaminA() + (food.getVitaminA() * m.getQuantity() * 1000D / 100D));
						total.setVitaminB1(
								total.getVitaminB1() + (food.getVitaminB1() * m.getQuantity() * 1000D / 100D));
						total.setVitaminB2(
								total.getVitaminB2() + (food.getVitaminB1() * m.getQuantity() * 1000D / 100D));
						total.setVitaminB6(
								total.getVitaminB6() + (food.getVitaminB6() * m.getQuantity() * 1000D / 100D));
						total.setVitaminB12(
								total.getVitaminB12() + (food.getVitaminB12() * m.getQuantity() * 1000D / 100D));
						total.setVitaminC(total.getVitaminC() + (food.getVitaminC() * m.getQuantity() * 1000D / 100D));
						total.setVitaminE(total.getVitaminE() + (food.getVitaminE() * m.getQuantity() * 1000D / 100D));
						total.setNicotinicCid(
								total.getNicotinicCid() + (food.getNicotinicCid() * m.getQuantity() * 1000D / 100D));
						total.setRetinol(total.getRetinol() + (food.getRetinol() * m.getQuantity() * 1000D / 100D));
						total.setCalcium(total.getCalcium() + (food.getCalcium() * m.getQuantity() * 1000D / 100D));
						total.setIron(total.getIron() + (food.getIron() * m.getQuantity() * 1000D / 100D));
						total.setZinc(total.getZinc() + (food.getZinc() * m.getQuantity() * 1000D / 100D));
						total.setIodine(total.getIodine() + (food.getIodine() * m.getQuantity() * 1000D / 100D));
						total.setPhosphorus(
								total.getPhosphorus() + (food.getPhosphorus() * m.getQuantity() * 1000D / 100D));
					}

					double totalPersonHE = totalHeatEnergy / totalPerson;
					fat = fat / (days * totalPerson);
					protein = protein / (days * totalPerson);
					carbohydrate = carbohydrate / (days * totalPerson);

					FoodComposition avg = new FoodComposition();
					avg.setQuantity(total.getQuantity() / (days * totalPerson));
					avg.setHeatEnergy(totalPersonHE);
					avg.setFat(fat);
					avg.setProtein(protein);
					avg.setCarbohydrate(carbohydrate);
					avg.setVitaminA(total.getVitaminA() / (days * totalPerson));
					avg.setVitaminB1(total.getVitaminB1() / (days * totalPerson));
					avg.setVitaminB2(total.getVitaminB2() / (days * totalPerson));
					avg.setVitaminB6(total.getVitaminB6() / (days * totalPerson));
					avg.setVitaminB12(total.getVitaminB12() / (days * totalPerson));
					avg.setVitaminC(total.getVitaminC() / (days * totalPerson));
					avg.setVitaminE(total.getVitaminE() / (days * totalPerson));
					avg.setNicotinicCid(total.getNicotinicCid() / (days * totalPerson));
					avg.setRetinol(total.getRetinol() / (days * totalPerson));
					avg.setCalcium(total.getCalcium() / (days * totalPerson));
					avg.setIron(total.getIron() / (days * totalPerson));
					avg.setZinc(total.getZinc() / (days * totalPerson));
					avg.setIodine(total.getIodine() / (days * totalPerson));
					avg.setPhosphorus(total.getPhosphorus() / (days * totalPerson));
					...
					params.put("avg", avg);
					params.put("fat", avg.getFat());
					params.put("fatHE", avg.getFat() * 9);
					params.put("protein", avg.getProtein());
					params.put("proteinHE", avg.getProtein() * 4);
					params.put("carbohydrate", carbohydrate);
					params.put("carbohydrateHE", totalPersonHE - avg.getFat() * 9 - avg.getProtein() * 4);

					logger.debug("days size:" + days);
					...
					params.put("carbohydratePercent", 100
							- (avg.getFat() * 9 / totalPersonHE * 100D + avg.getProtein() * 4 / totalPersonHE * 100D));

					beansProtein = beansProtein * 1000 / (days * totalPerson);
					animalsProtein = animalsProtein * 1000 / (days * totalPerson);

					params.put("beansProtein", beansProtein);
					params.put("beansProteinHE", beansProtein * 4);
					params.put("animalProtein", animalsProtein);
					params.put("animalProteinHE", animalsProtein * 4);
					...
					if (foodDRI != null) {
						...
						String fatStd = "";
						fatStd = Math.round(foodDRI.getHeatEnergy() / 9 * 0.2) + "~"
								+ Math.round(foodDRI.getHeatEnergy() / 9 * 0.35);
						params.put("fatStd", fatStd);

						String proteinStd = "";
						proteinStd = Math.round(foodDRI.getHeatEnergy() / 4 * 0.12) + "~"
								+ Math.round(foodDRI.getHeatEnergy() / 4 * 0.15);
						params.put("proteinStd", proteinStd);

						String carbohydrateStd = "";
						carbohydrateStd = Math.round(foodDRI.getHeatEnergy() / 4 * 0.5) + "~"
								+ Math.round(foodDRI.getHeatEnergy() / 4 * 0.55);
