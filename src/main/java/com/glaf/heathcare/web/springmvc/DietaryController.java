...
public class DietaryController {
	...
	public byte[] batchAdd(HttpServletRequest request) {
		...
		if (loginContext.isSystemAdministrator() || loginContext.getRoles().contains("HealthPhysician")
			...
			String iKey = "dietary_copy_" + loginContext.getTenantId();
			if (ContextUtil.get(iKey) == null) {
				try {
					logger.debug("正在复制食谱模板......");
					ContextUtil.put(iKey, loginContext.getActorId());
					for (int i = 1; i <= 5; i++) {
						query.dayOfWeek(i);
	...
	public byte[] calculateAll(HttpServletRequest request) {
		...
		String objectIds = request.getParameter("objectIds");
		if (StringUtils.isNotEmpty(objectIds)) {
			String iKey = "dietary_calculate_" + loginContext.getTenantId();
			if (ContextUtil.get(iKey) == null) {
				try {
					logger.debug("正在计算食谱成分......");
					ContextUtil.put(iKey, loginContext.getActorId());
					DietaryBean bean = new DietaryBean();
	...
	public byte[] executeCount(HttpServletRequest request) {
		...
		if (loginContext.getRoles().contains("HealthPhysician") || loginContext.getRoles().contains("TenantAdmin")) {
			int year = RequestUtils.getInt(request, "year");
			int week = RequestUtils.getInt(request, "week");
			String iKey = "dietary_count_" + loginContext.getTenantId();
			if (ContextUtil.get(iKey) == null) {
				try {
					logger.debug("正在统计成分......");
					ContextUtil.put(iKey, loginContext.getActorId());
	...
	public byte[] saveBatch(HttpServletRequest request) {
		...
		if (loginContext.isSystemAdministrator() || loginContext.getRoles().contains("HealthPhysician")
			...
			String objectIds = request.getParameter("objectIds");
			int week = RequestUtils.getInt(request, "week");
			String iKey = "dietary_copy4_" + loginContext.getTenantId();
			if (ContextUtil.get(iKey) == null) {
				try {
					logger.debug("准备复制模板到实例......");
					ContextUtil.put(iKey, loginContext.getActorId());
					if (StringUtils.isNotEmpty(objectIds)) {
	...
	@ResponseBody
	@RequestMapping("/updateDietaryName")
	public byte[] updateDietaryName(HttpServletRequest request) {
		LoginContext loginContext = RequestUtils.getLoginContext(request);
		String actorId = loginContext.getActorId();
		...
		try {
			if (dietaryId > 0) {
				dietary = dietaryService.getDietary(loginContext.getTenantId(), dietaryId);
			}
			if (dietary != null) {
				if (!(loginContext.getRoles().contains("TenantAdmin")
						|| loginContext.getRoles().contains("HealthPhysician")
						|| loginContext.getRoles().contains("StockManager"))) {
					if (!StringUtils.equals(dietary.getCreateBy(), actorId)) {
						return ResponseUtils.responseJsonResult(false, "您没有修改该数据的权限。");
					}
				}
				dietary.setName(request.getParameter("dietaryName"));
				dietary.setUpdateBy(actorId);
				this.dietaryService.updateDietaryName(dietary);
				return ResponseUtils.responseJsonResult(true);
			}
		} catch (Exception ex) {
			logger.error(ex);
