...
public class GoodsPurchasePlanController {
	...
	public byte[] cancelAudit(HttpServletRequest request) {
		LoginContext loginContext = RequestUtils.getLoginContext(request);
		long id = RequestUtils.getLong(request, "id");
		GoodsPurchasePlan model = null;
		try {
			if (loginContext.isTenantAdmin()) {
				if (id > 0) {
					model = goodsPurchasePlanService.getGoodsPurchasePlan(loginContext.getTenantId(), id);
					if (model != null && DateUtils.getDaysBetween(model.getCreateTime(), new Date()) <= 30) {
						...
						goodsPurchasePlanService.updateGoodsPurchasePlanStatus(model);
						return ResponseUtils.responseJsonResult(true);
					}
				}
			}
		...
		if (goodsPurchasePlan != null) {
			...
			if (loginContext.isTenantAdmin()) {
				if (goodsPurchasePlan.getBusinessStatus() == 9
						&& DateUtils.getDaysBetween(goodsPurchasePlan.getCreateTime(), new Date()) <= 30) {
