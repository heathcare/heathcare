...
public class GoodsAcceptanceController {
						...
						if (goodsAcceptance != null) {
							...
							if ((total1 - total2) < 0) {
								return ResponseUtils.responseJsonResult(false,
										goodsAcceptance.getGoodsName() + "入库数量不能超过采购数量。");
							}
	...
	public byte[] cancelAudit(HttpServletRequest request) {
		LoginContext loginContext = RequestUtils.getLoginContext(request);
		long id = RequestUtils.getLong(request, "id");
		GoodsAcceptance model = null;
		try {
			if (loginContext.isTenantAdmin()) {
				if (id > 0) {
					model = goodsAcceptanceService.getGoodsAcceptance(loginContext.getTenantId(), id);
					if (model != null && DateUtils.getDaysBetween(model.getCreateTime(), new Date()) <= 30) {
						...
						goodsAcceptanceService.updateGoodsAcceptanceStatus(model);
						return ResponseUtils.responseJsonResult(true);
					}
				}
			}
	...
	public ModelAndView edit(HttpServletRequest request, ModelMap modelMap) {
		...
		if (goodsAcceptance != null) {
			...
			if (loginContext.isTenantAdmin()) {
				if (goodsAcceptance.getBusinessStatus() == 9
						&& DateUtils.getDaysBetween(goodsAcceptance.getCreateTime(), new Date()) <= 30) {
			...
			try {
				if (nodeId > 0) {
					...
					if (list != null && !list.isEmpty()) {
						GoodsPurchaseQuery query1 = new GoodsPurchaseQuery();
						query1.tenantId(loginContext.getTenantId());
						query1.businessStatus(9);// 已经确定采购的物品
						query1.fullDay(DateUtils.getNowYearMonthDay());
						List<GoodsPurchase> goods1 = goodsPurchaseService.list(query1);
				...
				if (goodsId > 0) {
					...
					if (fd != null) {
						...
						if (goods1 != null && !goods1.isEmpty()) {
							...
							existsQty = Math.round(existsQty);
