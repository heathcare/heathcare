		...
		if (loginContext.getRoles().contains("HygieneOrg")) {
			...
			if (fullDay > 0) {
				...
				try {
					...
					StatsHistory his = statsHistoryService.getStatsHistory(id);
					if (his != null) {
						if (((System.currentTimeMillis() - his.getUpdateTime().getTime()) > DateUtils.MINUTE * 60)) {
							DayDietaryBean bean = new DayDietaryBean();
	...
	public ModelAndView showTenantExport(HttpServletRequest request, ModelMap modelMap) {
		...
		if (loginContext.getRoles().contains("HygieneOrg")) {
			...
			if (year > 0 && week > 0) {
				...
				if (concurrentMap.get("html_" + tenantId) != null) {
					try {
						Thread.sleep(500L);
					} catch (InterruptedException e) {
					}
				...
				try {
					concurrentMap.put("html_" + tenantId, "1");

					String id = "html_" + tenantId + "_" + year + "_" + SysConfig.getSemester() + "_" + week;
					StatsHistory his = statsHistoryService.getStatsHistory(id);
					if (his != null) {
						if (((System.currentTimeMillis() - his.getUpdateTime().getTime()) > DateUtils.MINUTE * 60)) {
							DietaryBean bean = new DietaryBean();
							bean.executeCountAll(tenantId, year, week);
							bean.executeCountItems(tenantId, year, week);
							his.setUpdateBy(loginContext.getActorId());
							his.setUpdateTime(new Date());
							statsHistoryService.save(his);
						}
					...
					if (year > 0 && week > 0) {
						if (loginContext.getRoles().contains("HygieneOrg")) {
							try {
								int semester = SysConfig.getSemester();
								DayDietaryStatisticsBean bean = new DayDietaryStatisticsBean();
								bean.execute(tenantId, year, semester, week);
							} catch (Exception ex) {
							}
						}
					}
					...
					concurrentMap.remove("html_" + tenantId);
				}
			}
		}
