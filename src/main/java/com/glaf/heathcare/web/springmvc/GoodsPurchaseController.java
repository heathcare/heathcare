...
public class GoodsPurchaseController {
	...
	public byte[] cancelAudit(HttpServletRequest request) {
		LoginContext loginContext = RequestUtils.getLoginContext(request);
		long id = RequestUtils.getLong(request, "id");
		GoodsPurchase model = null;
		try {
			if (loginContext.isTenantAdmin()) {
				if (id > 0) {
					model = goodsPurchaseService.getGoodsPurchase(loginContext.getTenantId(), id);
					if (model != null && DateUtils.getDaysBetween(model.getCreateTime(), new Date()) <= 30) {
						...
						goodsPurchaseService.updateGoodsPurchaseStatus(model);
						return ResponseUtils.responseJsonResult(true);
					}
				}
			}
	...
	public byte[] copyPurchasePlan(HttpServletRequest request) {
		...
		String iKey = "purchase_copy4_" + loginContext.getTenantId();
		if (ContextUtil.get(iKey) == null) {
			try {
				logger.debug("准备复制采购计划到采购单......");
			...
			if (goodsPurchase != null) {
				...
				if (goodsPurchase.getGoodsId() > 0) {
					FoodComposition foodComposition = foodCompositionService
							.getFoodComposition(goodsPurchase.getGoodsId());
				...
				if (loginContext.isTenantAdmin()) {
					if (goodsPurchase.getBusinessStatus() == 9
							&& DateUtils.getDaysBetween(goodsPurchase.getCreateTime(), new Date()) <= 30) {
