...
public class GoodsOutStockController {
	...
	public byte[] cancelAudit(HttpServletRequest request) {
		LoginContext loginContext = RequestUtils.getLoginContext(request);
		long id = RequestUtils.getLong(request, "id");
		GoodsOutStock model = null;
		try {
			if (loginContext.isTenantAdmin()) {
				if (id > 0) {
					model = goodsOutStockService.getGoodsOutStock(loginContext.getTenantId(), id);
					if (model != null && model.getBusinessStatus() == 9
							&& DateUtils.getDaysBetween(model.getCreateTime(), new Date()) <= 30) {
						goodsOutStockService.cancelAudit(model);
						return ResponseUtils.responseJsonResult(true);
					}
				}
			}
	...
	public byte[] copyPurchase(HttpServletRequest request) {
		...
		int fullDay = DateUtils.getYearMonthDay(dateTime);
		String iKey = "outstock_purchase_copy_" + loginContext.getTenantId();
		if (ContextUtil.get(iKey) == null) {
			try {
				logger.debug("准备复制采购到出库......");
				ContextUtil.put(iKey, loginContext.getActorId());
		...
		if (goodsOutStock != null) {
			...
			if (loginContext.isTenantAdmin()) {
				if (goodsOutStock.getBusinessStatus() == 9
						&& DateUtils.getDaysBetween(goodsOutStock.getCreateTime(), new Date()) <= 30) {
