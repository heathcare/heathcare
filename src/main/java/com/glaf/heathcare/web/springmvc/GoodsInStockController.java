...
public class GoodsInStockController {
			...
			try {
				...
				if (goodsInStock != null) {
					...
					if (!StringUtils.equals(tenant.getDisableDataConstraint(), "Y")) {
						GoodsAcceptanceQuery query1 = new GoodsAcceptanceQuery();
						query1.tenantId(loginContext.getTenantId());
						query1.businessStatus(9);// 已经确定验收的物品
						query1.setGoodsId(goodsId);
						query1.year(year);
						query1.month(month);
						List<GoodsAcceptance> goods1 = goodsAcceptanceService.list(query1);
						...
						query2.year(year);
						query2.month(month);
						List<GoodsInStock> goods2 = goodsInStockService.list(query2);

						double total1 = 0;// 总验收量
						for (GoodsAcceptance g : goods1) {
							total1 = total1 + g.getQuantity();
						}

						double total2 = 0;// 总入库量
						for (GoodsInStock g : goods2) {
							total2 = total2 + g.getQuantity();
						}

						if ((total1 - total2) < 0) {
							return ResponseUtils.responseJsonResult(false,
									goodsInStock.getGoodsName() + "入库数量不能超过验收数量");
						}

						if (goodsInStock.getQuantity() > (total1 - total2)) {
							return ResponseUtils.responseJsonResult(false,
									"入库数量不能超过验收数量，入库数量不得超过" + (total1 - total2) + "。");
						}
					}
				...
				if (StringUtils.isNotEmpty(ids)) {
					...
					if (list != null && !list.isEmpty()) {
						...
						for (GoodsInStock p : list) {
							...
							SysTenant tenant = sysTenantService.getSysTenantByTenantId(loginContext.getTenantId());
							if (!StringUtils.equals(tenant.getDisableDataConstraint(), "Y")) {
								calendar.setTime(p.getInStockTime());
								int year = calendar.get(Calendar.YEAR);
								int month = calendar.get(Calendar.MONTH) + 1;
								...
								if ((total1 - total2) < 0) {
									return ResponseUtils.responseJsonResult(false, p.getGoodsName() + "入库数量不能超过验收数量。");
								}

								if (p.getQuantity() > (total1 - total2)) {
									return ResponseUtils.responseJsonResult(false,
											p.getGoodsName() + "入库数量不能超过验收数量，入库数量不得超过" + (total1 - total2) + "。");
								}
							}
	...
	public byte[] copyPurchase(HttpServletRequest request) {
		...
		String iKey = "instock_copy_" + loginContext.getTenantId();
		if (ContextUtil.get(iKey) == null) {
			try {
				logger.debug("准备复制采购到入库......");
			...
			if (list != null && !list.isEmpty()) {
				...
				for (GoodsInStock goodsInStock : list) {
					...
					rowJSON.put("ex_secutity_id", RSAUtils.encryptString(
							loginContext.getTenantId() + ":" + String.valueOf(goodsInStock.getId())));
		...
		if (loginContext.getRoles().contains("TenantAdmin") || loginContext.getRoles().contains("Buyer")
			...
			try {
				...
				if (!StringUtils.equals(tenant.getDisableDataConstraint(), "Y")) {
					...
					if ((total1 - total2) < 0) {
						return ResponseUtils.responseJsonResult(false, "入库数量不能超过验收数量");
					}
				...
				goodsInStock.setGoodsId(goodsId);
			...
			if (StringUtils.isNotEmpty(json)) {
				try {
					...
					if (array != null && array.size() > 0) {
						...
						Map<Long, GoodsInStock> dataMap = new HashMap<Long, GoodsInStock>();
						for (int i = 0; i < len; i++) {
						...
						if (!dataMap.isEmpty()) {
							goodsInStockService.updateAll(loginContext.getTenantId(), dataMap);
							return ResponseUtils.responseJsonResult(true);
						}
					}
				} catch (Exception ex) {
