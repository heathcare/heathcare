...
import com.glaf.heathcare.util.PersonJsonFactory;
...
public class GradeInfoController {
	...
	@ResponseBody
	public byte[] personJson(HttpServletRequest request) throws IOException {
		LoginContext loginContext = RequestUtils.getLoginContext(request);
		Map<String, Object> params = RequestUtils.getParameterMap(request);
		...
		return PersonJsonFactory.listToArray(list).toJSONString().getBytes("UTF-8");
	}
