...
public class MedicalExaminationXlsImporter {
	protected final static Log logger = LogFactory.getLog(MedicalExaminationXlsImporter.class);
	...
	public String importData(String tenantId, String gradeId, String type, Date checkDate,
		...
		logger.debug("----------------MedicalExaminationXlsReader------------------");
		...
		for (MedicalExaminationXlsArea area : areas) {
			...
			for (int rowIndex = area.getStartRow(); rowIndex <= endRow; rowIndex++) {
				...
				try {
					...
					if (cell != null) {
						cellValue = ExcelUtils.getValue(cell, 0);
