...
public class GoodsInStockServiceImpl implements GoodsInStockService {
	...
	@Transactional
	public void cancelAudit(GoodsInStock goodsInStock) {
		if (goodsInStock != null && goodsInStock.getBusinessStatus() == 9) {// 必须是审核过的
			...
			goodsStockService.outStock(goodsStock);// 减库存量

			goodsInStock.setBusinessStatus(0);
			goodsInStock.setConfirmTime(null);
			goodsInStock.setConfirmBy(null);
