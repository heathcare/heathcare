...
public class GoodsPurchaseServiceImpl implements GoodsPurchaseService {
	 ...
	 * @return
	 */
	public Map<Long, Double> getPriceMap(String tenantId, int fullDay) {
		...
		if (list != null && !list.isEmpty()) {
			for (GoodsPurchase p : list) {
				priceMap.put(p.getGoodsId(), p.getPrice());
			}
		}
		return priceMap;
	}
	 ...
	 * @return
	 */
	public Map<Long, Double> getTotalPriceMap(String tenantId, int fullDay) {
		Map<Long, Double> priceMap = new HashMap<Long, Double>();
		GoodsPurchaseQuery query = new GoodsPurchaseQuery();
		query.tenantId(tenantId);
		query.fullDay(fullDay);
		...
		if (list != null && !list.isEmpty()) {
			for (GoodsPurchase p : list) {
				priceMap.put(p.getGoodsId(), p.getTotalPrice());
			}
		}
		return priceMap;
	}
