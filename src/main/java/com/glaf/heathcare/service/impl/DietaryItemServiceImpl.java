...
public class DietaryItemServiceImpl implements DietaryItemService {
	...
	private List<DietaryTemplate> getDietaryTemplates(int suitNo, String sysFlag) {
	...
	public List<DietaryItem> getSysDietaryItems(int suitNo) {
		StringBuilder buffer = new StringBuilder();
		buffer.append("health_di_list2_").append(suitNo);
		String cacheKey = buffer.toString();
		if (SystemConfig.getBoolean("use_query_cache")) {
			String text = CacheFactory.getString("health_di_list2", cacheKey);
			if (StringUtils.isNotEmpty(text)) {
				try {
					JSONArray array = JSON.parseArray(text);
					return DietaryItemJsonFactory.arrayToList(array);
		...
		List<DietaryTemplate> list = getDietaryTemplates(suitNo, "Y");
		...
		List<DietaryItem> allItems = list(query2);
		if (allItems != null && !allItems.isEmpty()) {
			if (SystemConfig.getBoolean("use_query_cache")) {
				CacheFactory.put("health_di_list2", cacheKey,
						DietaryItemJsonFactory.listToArray(allItems).toJSONString());
			}
		}
		return allItems;
	}
