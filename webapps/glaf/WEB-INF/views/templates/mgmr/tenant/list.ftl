...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/mgmr/tenant/json?provinceId=${provinceId}&cityId=${cityId}&areaId=${areaId}&level=${level}&property=${property}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
	...
	function formatterKeys(val, row){
		var str = "<a href='javascript:editRow(\""+row.tenantId+"\");'>修改</a>&nbsp;<a href='javascript:users(\""+row.tenantId+"\");'>用户</a>&nbsp;<a href='javascript:tcImage(\""+row.tenantId+"\");'>营业执照</a>";
	    return str;
	}

	function exp(tenantId){
		window.open('${contextPath}/mgmr/tenantExport/export?tenantId='+tenantId);
	}
	...
	function addNew(){
	    var link="${contextPath}/mgmr/tenant/edit";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function users(tenantId){
	    var link="${contextPath}/mgmr/tenant/user?tenantId="+tenantId;
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/mgmr/tenant/edit?tenantId='+row.tenantId;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
   ...
   function editRow(tenantId){
	    var link = '${contextPath}/mgmr/tenant/edit?tenantId='+tenantId;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function searchWin(){
	    jQuery('#dlg').dialog('open').dialog('setTitle','客户信息查询');
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/mgmr/tenant/edit?tenantId='+selected.tenantId;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function users(){
	    ...
	    var selected = jQuery('#mydatagrid').datagrid('getSelected');
	    if (selected ){
		  var link = '${contextPath}/mgmr/tenant/user?tenantId='+selected.tenantId;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function searchData(){
        ...
        jQuery.ajax({
                    type: "POST",
                    url: '${contextPath}/mgmr/tenant/json',
                    dataType:  'json',
                    data: params,
                    error: function(data){
                              alert('服务器处理错误！');
    ...
    <table>
      <tr>
	    <td>
			...
			&nbsp;<span class="x_content_title">客户信息列表</span>
