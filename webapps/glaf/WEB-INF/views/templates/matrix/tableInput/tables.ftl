...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/matrix/tableInput/json?type=${type}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
   ...
   function formatterKeys(val, row){
	...
	function addNew(){
	    var link="${contextPath}/matrix/tableInput/editTable?type=${type}";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function tableColumns(tableId){
		link="${contextPath}/matrix/tableInput/columns?type=${type}&tableId="+tableId;
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function editRow(tableId){
		link="${contextPath}/matrix/tableInput/editTable?type=${type}&tableId="+tableId;
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/matrix/tableInput/editTable?type=${type}&tableId='+row.tableId;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(tableId){
		if(confirm("删除数据不能恢复，确定删除吗？")){
			if(confirm("表相关的列定义数据都将删除且不能恢复，确实要删除吗？")){
			  jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/matrix/tableInput/deleteTable?tableId='+tableId,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function updateSchema(tableId){
		if(confirm("将根据字段信息更新数据库物理表结构，确定吗？")){
			if(confirm("字段类型确定后将无法修改，确实要更新吗？")){
			  jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/matrix/tableInput/updateSchema?tableId='+tableId,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function updateAllSchema(){
		if(confirm("将根据字段信息更新数据库物理表结构，确定吗？")){
			if(confirm("字段类型确定后将无法修改，确实要更新吗？")){
			  jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/matrix/tableInput/updateAllSchema',
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/matrix/tableInput/editTable?type=${type}&tableId='+selected.tableId;
		  jQuery.layer({
				type: 2,
				maxmin: true,
				shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length == 1 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/matrix/tableInput/deleteTable?type=${type}&tableId='+rows[0].tableId,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function showDB(){
        var link='${contextPath}/matrix/tableInput/databases';
		jQuery.layer({
				type: 2,
				maxmin: true,
				shadeClose: true,
...
<div class="easyui-layout" data-options="fit:true">  
   <div data-options="region:'north', split:false, border:true" style="height:42px" class="toolbar-backgroud"> 
    <div style="margin:4px;"  > 
		...
		&nbsp;<span class="x_content_title">数据导入定义列表</span>
