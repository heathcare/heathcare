...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/matrix/tableInput/columnsJson?tableId=${tableId}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
					...
					{title:'取数位置', field:'position', align:'right', width:80},
   ...
   function formatterKeys(val, row){
	...
	function addNew(){
	    var link="${contextPath}/matrix/tableInput/editColumn?tableId=${tableId}";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function editRow(id){
		link="${contextPath}/matrix/tableInput/editColumn?tableId=${tableId}&id="+id;
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/matrix/tableInput/editColumn?tableId=${tableId}&id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("删除数据不能恢复，确定删除吗？")){
			if(confirm("该表相关的列定义数据都将删除且不能恢复，确实要删除吗？")){
			  jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/matrix/tableInput/deleteColumn?tableId=${tableId}&id='+id,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/matrix/tableInput/editColumn?tableId=${tableId}&id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/matrix/tableInput/deleteColumn?tableId=${tableId}&ids='+str,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
  ...
  function sortColumns(){
        link="${contextPath}/matrix/tableInput/showSort?tableId=${tableId}";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
