...
<script type="text/javascript">
    ...
    function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: "${contextPath}/matrix/tableInput/saveSort?items="+str,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
...
<body>
        ...
        <div id="example">
            ...
            <div id="pages">
                <div id="pages-title"><span>字段列表</span></div>
                <div id="sortable">
				<#list  columns as col>
                    <div class="item">
                        <span class="handler" title="选中后拖到指定位置">&nbsp;</span>
                        <label id="${col.id}" title="选中后拖到指定位置">${col.title} [${col.columnName}]</label>
                    </div>
				</#list> 
               </div>
			  <center>
