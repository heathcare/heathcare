...
<script type="text/javascript">
	...
	function saveData(){
	    ...
	    if(confirm("确定将"+dateTime+"的计划用量复制到实际用量表中吗？")){
		  var params = jQuery("#iForm").formSerialize();
		  jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsActualQuantity/copyPlan',
				   data: params,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
...
<div class="easyui-layout" data-options="fit:true">  
  <div data-options="region:'north',split:false,border:true" style="height:45px" class="toolbar-backgroud"> 
	...
	<img src="${contextPath}/static/images/window.png">&nbsp;<span class="x_content_title">复制计划用量</span>
  ...
  <table class="easyui-form" style="width:650px;" align="center">
	...
	<tr>
	  <td colspan="4">
	     <div style="font-size: 14px; color:blue; font-weight:bold; font-family: 微软雅黑; margin-top:20px;margin-left:80px;">
		   （提示：已经在实际用量表的食品不会再加入。）
		 </div>
	  </td>
	</tr>
