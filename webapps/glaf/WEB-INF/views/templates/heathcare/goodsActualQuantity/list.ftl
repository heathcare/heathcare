...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				columns:[[
					...
					{title:'重量', field:'quantity', width:90, align:'right', sortable:true, formatter:formatterQuantity},
					...
					{title:'使用时间', field:'usageTime', width:100, align:'center', sortable:true},
					{title:'星期', field:'usageTime_week', width:100, align:'center'},
	...
	function formatterQuantity2(val, row){
		if(row.businessStatus == 9){
			return val;
		}
        return "<input type='text' id='"+row.ex_secutity_id+"' name='plan_qty' size='5' class='x-small-decimal' value='"+val.replace("KG","")+"'>";
	}
	...
	function formatterQuantity(val, row){
        ...
        return "<input type='text' id='qty_"+row.ex_secutity_id+"' name='plan_qty' size='5' class='x-small-decimal' " + 
		        " onchange=javascript:changeTotalPrice(\""+row.ex_secutity_id+"\"); value='"+val.replace("KG","")+"'>";
	}
   ...
   function editRow(id){
    ...
    function copyInStock(){
		var link="${contextPath}/heathcare/goodsActualQuantity/showCopy4";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function copyPlan(){
		var link="${contextPath}/heathcare/goodsActualQuantity/showCopy3";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function doSearch(){
		...
		if(startTime != ""){
			//jQuery.cookie("startTime", startTime);
			startTime = startTime + " 00:00:00";
		}
		if(endTime != ""){
			//jQuery.cookie("endTime", endTime);
    ...
    function showToday(){
	   var avgQuantity = document.getElementById("avgQuantity").checked;
	   if(avgQuantity){
		   avgQuantity = "on";
	   }
	   //alert(avgQuantity);
	...
	function showMonthFee(){
	    var link = '${contextPath}/heathcare/goodsActualQuantity/cost';
        var x=90;
        var y=50;
        if(is_ie) {
        ...
        openWindow(link, self, x, y, 1280, 620);
	}
    ...
    function saveAll(){
	   if(confirm("确定保存数据吗？")){
		  ...
		  for(var i=0; i<len; i++){
              ...
              str = str +"{'id':'"+tmp+"', 'quantity':"+array[i].value.replace("KG","")
          ...
          jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsActualQuantity/updateAll',
				   data: params,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
...
<div class="easyui-layout" data-options="fit:true">  
   <div data-options="region:'north', split:false, border:true" style="height:76px" class="toolbar-backgroud"> 
      ...
      <table valign="top">
       <tr valign="top">
	    <td valign="top" width="48%">   
			...
			&nbsp;
			<a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-copy'" 
		       onclick="javascript:copyPlan();">复制计划用量</a>
			<br>
		...
		<td valign="middle" width="52%">
		  ...
		  <a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon_money'"
	         onclick="javascript:showMonthFee();">月度费用</a>
		  <br>
		</td>
       </tr>
      </table>
