...
<script type="text/javascript">
  <#if privilege_write == true>
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/personInspection/saveAll?inspectionFlag=${inspectionFlag}&gradeId=${gradeId}',
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
...
<div class="easyui-layout" data-options="fit:true">  
  ...
  <input type="hidden" id="inspectionFlag" name="inspectionFlag" value="${inspectionFlag}"/>
	...
	<tr>
		...
		<td  class="table-content" align="left">
		   ...
		   <#if person.status == 0>
		    ...
		    >
			处理情况&nbsp;
			<input id="treat_${person.id}" name="treat_${person.id}" style="width:380px;" 
				   class="easyui-validatebox  x-small-text"
				   value="${person.treat}" >
			<br><br>
			具体原因&nbsp;
		    <input id="remark_${person.id}" name="remark_${person.id}" style="width:380px;" 
				   class="easyui-validatebox  x-small-text"
				   value="${person.remark}" > 
		   </div>
		</td>
