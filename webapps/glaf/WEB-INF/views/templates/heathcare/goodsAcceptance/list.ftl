...
<script type="text/javascript">
    ...
    function getLink(){
	    var link_ = "${contextPath}/heathcare/goodsAcceptance/json?startTime=${startTime}&endTime=${endTime}";
		//alert(link_);
	    return link_;
	}
   ...
   function formatterKey(value, row, index) {
	...
	function addNew(){
	    var link="${contextPath}/heathcare/goodsAcceptance/edit";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/heathcare/goodsAcceptance/edit?audit=${audit}&id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
   ...
   function editRow(id){
	    var link = '${contextPath}/heathcare/goodsAcceptance/edit?id='+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function auditRow(id){
	    var link = '${contextPath}/heathcare/goodsAcceptance/edit?audit=true&id='+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsAcceptance/delete?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function onRowClick(rowIndex, row){
	    var link = '${contextPath}/heathcare/goodsAcceptance/edit?id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function searchWin(){
	    jQuery('#dlg').dialog('open').dialog('setTitle','物品验收单查询');
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/heathcare/goodsAcceptance/edit?id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function auditSelected(){
	    ...
	    var selected = jQuery('#mydatagrid').datagrid('getSelected');
	    if (selected ){
		  var link = '${contextPath}/heathcare/goodsAcceptance/edit?audit=true&id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    var link='${contextPath}/heathcare/goodsAcceptance/edit?readonly=true&id='+selected.id;
		    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function stocklist(){
        var link='${contextPath}/heathcare/goodsAcceptance/stocklist';
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsAcceptance/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function searchData(){
        ...
        jQuery.ajax({
                    type: "POST",
                    url: '${contextPath}/heathcare/goodsAcceptance/json',
                    dataType:  'json',
                    data: params,
                    error: function(data){
                              alert('服务器处理错误！');
	...
	function batchEdit(){
        var link = '${contextPath}/heathcare/goodsAcceptance/batchEdit';
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function auditBatch(){
	  ...
	  if(confirm("确定批量通过审核吗？")){
		if(confirm("审核通过后的数据不能再修改或删除，确定吗？")){
		  jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsAcceptance/audit?ids='+objectIds,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
    ...
    function doExport() {
        ...
        var link = '${contextPath}/heathcare/reportMain/exportXls?startTime='+startTime+'&endTime='+endTime+'&reportId=GoodsAcceptance';
		window.open(link);	
	}
	
	function copyPurchase(){
		var link="${contextPath}/heathcare/goodsAcceptance/showCopy";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
    ...
    function saveAll(){
	   if(confirm("确定保存数据吗？")){
          ...
          jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsAcceptance/updateAll',
				   data: params,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
