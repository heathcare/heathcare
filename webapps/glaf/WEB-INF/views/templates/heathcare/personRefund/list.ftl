...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/heathcare/personRefund/json?personId=${personId}&gradeId=${gradeId}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
						...
						{title:'退费金额', field:'money', width:100},
						{title:'退费时间', field:'refundTime', width:120},
	...
	function addNew(){
	    var link="${contextPath}/heathcare/personRefund/edit?personId=${personId}&gradeId=${gradeId}";
		layer.open({
		  type: 2,
          maxmin: true,
		  shadeClose: true,
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/heathcare/personRefund/edit?id='+row.id;
		layer.open({
		  type: 2,
          maxmin: true,
		  shadeClose: true,
    ...
    function editRow(id){
	    var link = '${contextPath}/heathcare/personRefund/edit?id='+id;
		layer.open({
		  type: 2,
          maxmin: true,
		  shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/personRefund/delete?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function searchWin(){
	    jQuery('#dlg').dialog('open').dialog('setTitle','退费信息查询');
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/heathcare/personRefund/edit?id='+selected.id;
		  layer.open({
			  type: 2,
			  maxmin: true,
			  shadeClose: true,
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    var link='${contextPath}/heathcare/personRefund/edit?readonly=true&id='+selected.id;
		    layer.open({
			  type: 2,
			  maxmin: true,
			  shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/personRefund/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function searchData(){
        ...
        jQuery.ajax({
                    type: "POST",
                    url: '${contextPath}/heathcare/personRefund/json',
                    dataType:  'json',
                    data: params,
                    error: function(data){
                              alert('服务器处理错误！');
	...
	function auditSelected(){
	    ...
	    var selected = jQuery('#mydatagrid').datagrid('getSelected');
	    if (selected ){
		  var link = '${contextPath}/heathcare/personRefund/edit?audit=true&id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function auditBatch(){
	  ...
	  if(confirm("确定批量通过审核吗？")){
		if(confirm("审核通过后的数据不能再修改或删除，确定吗？")){
		  jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/personRefund/audit?ids='+objectIds,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function payList(){
		location.href="${contextPath}/heathcare/personPayment?personId=${personId}&gradeId=${gradeId}&audit=${audit}";
	}
</script>
</head>
...
<div class="easyui-layout" data-options="fit:true">  
   <div data-options="region:'north', split:false, border:true" style="height:48px" class="toolbar-backgroud"> 
    <div style="margin:4px;"> 
		...
		&nbsp;<span class="x_content_title">退费信息列表</span>
		<#if audit == true>
		...
		<button type="button" id="searchButton" class="btn btnGrayMini" style="width: 90px" 
				onclick="javascript:payList();">缴费信息</button>
   </div> 
  </div> 
