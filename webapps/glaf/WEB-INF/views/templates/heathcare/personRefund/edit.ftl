...
<script type="text/javascript">
	...
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/personRefund/savePersonRefund',
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
...
<div class="easyui-layout" data-options="fit:true">  
  <div data-options="region:'north',split:true,border:true" style="height:45px" class="toolbar-backgroud"> 
	...
	<img src="${contextPath}/static/images/window.png"><span class="x_content_title">&nbsp;编辑退费信息</span>
	<#if audit == true>
	  <#if personRefund.businessStatus == 0>
	   <a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-save'" 
	      onclick="javascript:audit();" >确认通过</a> 
	  <#elseif personRefund.businessStatus == 9>
	  &nbsp;<span style="font:bold 13px 宋体; color:#ff0000;">已确认</span>&nbsp;
	  </#if>
	<#else>
	  <#if personRefund.businessStatus == 9>
	  &nbsp;<span style="font:bold 13px 宋体; color:#ff0000;">已确认</span>&nbsp;
	  <#else>
	  <a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-save'" 
  ...
  <input type="hidden" id="id" name="id" value="${personRefund.id}"/>
  <#if person?exists >
	...
	<tr>
		...
		<td align="left">
            <select id="type" name="type">
			 ...
			 <script type="text/javascript">
			     document.getElementById("type").value="${personRefund.type}";
			 </script>
		</td>
	</tr>
	<tr>
		<td width="20%" align="left">退费金额</td>
		<td align="left">
			<input id="money" name="money" type="text" style="width:80px; text-align:right;"
			       class="easyui-numberbox  x-text"  precision="2" 
				   value="${personRefund.money}"/>
		</td>
	</tr>
	<tr>
		<td width="20%" align="left">退费时间</td>
		<td align="left">
			<input id="refundTime" name="refundTime" type="text" 
			       class="easyui-datebox x-text" style="width:100px;"
			       <#if personRefund.refundTime?exists>
				   value="${personRefund.refundTime ? string('yyyy-MM-dd')}"
				   </#if>
				   />
		</td>
	</tr>
	<tr>
		...
		<td align="left">
		    <textarea id="remark" name="remark" 
			          ...
			          style="width:350px;height:120px">${personRefund.remark}</textarea>
		</td>
	</tr>
    </tbody>
  </table>
