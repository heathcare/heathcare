...
<script type="text/javascript">
	...
	var setting = {
		async: {
				enable: true,
				url:"${contextPath}/heathcare/gradeInfo/treeJson2?tenantId=${tenantId}"
			},
			callback: {
	...
	function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		if(treeNode.type == "person"){
		    ...
		    loadData('${contextPath}/heathcare/medicalExamination/reviewJson?checkId=${checkId}&tenantId=${tenantId}&type=${type}&personId='+treeNode.id);
		} else {
			jQuery("#nodeId").val("");
			jQuery("#personId").val("");
			jQuery("#div_person_print").hide();
			document.getElementById("gradeId").value=treeNode.id;
		    loadData('${contextPath}/heathcare/medicalExamination/reviewJson?checkId=${checkId}&type=${type}&tenantId=${tenantId}&gradeId='+treeNode.id);
		}
	}
	...
	function getLink(){
       var link_ = "${contextPath}/heathcare/medicalExamination/reviewJson?type=${type}&checkId=${checkId}&tenantId=${tenantId}";
	...
	function showPersonExamList(personId){
		var link='${contextPath}/heathcare/medicalExamination/examList?gradeId=${gradeId}&tenantId=${tenantId}&personId='+personId;
		var x=180;
        var y=80;
        if(is_ie) {
	...
	function addNew(){
	    ...
	    var link="${contextPath}/heathcare/medicalExamination/edit?personId="+personId+"&checkId=${checkId}&type=${type}&tenantId=${tenantId}";
		layer.open({
		  type: 2,
          maxmin: true,
		  shadeClose: true,
	...
	function searchData(){
        ...
        jQuery.ajax({
                    type: "POST",
                    url: '${contextPath}/heathcare/medicalExamination/reviewJson',
                    dataType:  'json',
                    data: params,
                    error: function(data){
                              alert('服务器处理错误！');
	...
	function showChart(){
        ...
        var month = jQuery("#month").val();
		var sex = jQuery("#sex").val();
		var link="${contextPath}/heathcare/medicalExaminationChart/columnLine?type=${type}&tenantId=${tenantId}&ts=${ts}";
		if(year != ""){
			link = link + "&year=" + year;
		}
		if(month != ""){
