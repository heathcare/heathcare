...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/heathcare/dietaryItem/templateJson?templateId=${templateId}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/heathcare/dietaryItem/edit?templateId=${templateId}&id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function addNew(){
	    var link="${contextPath}/heathcare/dietaryItem/edit?templateId=${templateId}";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
   ...
   function editRow(id){
	    var link = '${contextPath}/heathcare/dietaryItem/edit?templateId=${templateId}&id='+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/dietaryItem/deleteTemplateItem?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/heathcare/dietaryItem/edit?templateId=${templateId}&id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/dietaryItem/deleteTemplateItem?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
    ...
    function updateDietaryTemplateName(){
		var form = document.getElementById("iForm");
	    var link = "${contextPath}/heathcare/dietaryTemplate/updateDietaryTemplateName";
    ...
    <div style="margin:4px;"> 
	  <table width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
       <tr>
        ...
        <td width="50%">
			<#if canEditDietaryName == true>
			<form method="post" id="iForm" name="iForm" action="">
			<input type="hidden" id="dietaryTemplateId" name="dietaryTemplateId" value="${dietaryTemplate.id}">
			<input type="text" id="dietaryName" name="dietaryName" value="${dietaryTemplate.name}" class="x-small-text" 
			       style="width:250px">
			&nbsp;
			<input type="button" value="食谱改名" onclick="javascript:updateDietaryTemplateName();" class="btnGrayMini">
			</form>
			</#if>
		</td>
	   </tr>
