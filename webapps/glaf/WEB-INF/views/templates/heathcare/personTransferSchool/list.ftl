...
<script type="text/javascript">
    ...
    jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/heathcare/personTransferSchool/json?type=${type}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
						...
						{title:'转入前就读幼儿园', field:'fromSchool', width:200},
						{title:'转到幼儿园', field:'toSchool', width:200},
						{title:'转出日期', field:'leaveDate', width:100},
	...
	function addNew(){
	    var link="${contextPath}/heathcare/personTransferSchool/edit?type=${type}";
		layer.open({
		  type: 2,
          maxmin: true,
		  shadeClose: true,
	...
	function exportData(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected){
		  var link="${contextPath}/heathcare/reportMain/exportXls?reportId=PersonTransferSchool&ts=${ts}&pid="+selected.id;
          window.open(link);
		}
	}
   ...
   <div data-options="region:'north', split:false, border:true" style="height:42px" class="toolbar-backgroud"> 
    <div style="margin:4px;"> 
		...
		<a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon_export_xls'"
		   onclick="javascript:exportData();">导出</a>
   </div> 
  </div> 
