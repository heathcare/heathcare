...
<script type="text/javascript">
    ...
    function getLink(){
	    var link_ = "${contextPath}/heathcare/goodsPurchase/json?startTime=${startTime}&endTime=${endTime}";
		//alert(link_);
	    return link_;
	}
    ...
    jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/heathcare/goodsPurchase/json?startTime=${startTime}&endTime=${endTime}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
					...
					{title:'采购日期', field:'purchaseTime', width:100, align:'center', sortable:true},
	...
	function copyPlan(){
	    var link="${contextPath}/heathcare/goodsPurchase/showCopy";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function addNew(){
	    var link="${contextPath}/heathcare/goodsPurchase/edit";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/heathcare/goodsPurchase/edit?audit=${audit}&id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
   ...
   function editRow(id){
	    var link = '${contextPath}/heathcare/goodsPurchase/edit?id='+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function auditRow(id){
	    var link = '${contextPath}/heathcare/goodsPurchase/edit?audit=true&id='+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsPurchase/delete?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function onRowClick(rowIndex, row){
	    var link = '${contextPath}/heathcare/goodsPurchase/edit?id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/heathcare/goodsPurchase/edit?id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function auditSelected(){
	    ...
	    var selected = jQuery('#mydatagrid').datagrid('getSelected');
	    if (selected ){
		  var link = '${contextPath}/heathcare/goodsPurchase/edit?audit=true&id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    var link='${contextPath}/heathcare/goodsPurchase/edit?readonly=true&id='+selected.id;
		    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsPurchase/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function auditBatch(){
	  ...
	  if(confirm("确定批量通过审核吗？")){
		if(confirm("审核通过后的数据不能再修改或删除，确定吗？")){
		  jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsPurchase/audit?ids='+objectIds,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
    ...
    function exportXls(){
		...
		var link="${contextPath}/heathcare/reportMain/exportXls?reportId=GoodsPurchase";
		if(startTime != ""){
	...
	function doWeeklyExport(){
		...
		var link="${contextPath}/heathcare/reportMain/exportXls?reportId=WeeklyGoodsPurchase";
		if(startTime != ""){
    ...
    function saveAll(){
	   if(confirm("确定保存数据吗？")){
          ...
          jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsPurchase/updateAll',
				   data: params,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
...
<div class="easyui-layout" data-options="fit:true">  
   <div data-options="region:'north',split:false, border:true" style="height:48px" class="toolbar-backgroud"> 
     ...
     <tr>
	  <td valign="middle">
		...
		<a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-copy'" 
		   onclick="javascript:copyPlan();">复制采购计划</a>
	...
	<td valign="middle">
		  &nbsp;日期&nbsp;开始&nbsp;
		  <input id="startTime" name="startTime" type="text" class="easyui-datebox x-text" style="width:110px"
		         <#if startTime?exists> value="${startTime}"</#if>>
		  &nbsp;结束&nbsp;
		  <input id="endTime" name="endTime" type="text" class="easyui-datebox x-text" style="width:110px"
	...
	<td valign="middle">
		...
		<a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon_export_xls'"
	       onclick="javascript:doWeeklyExport();">一周采购单</a>
	</td>
    </tr>
   </table>
   </form>
