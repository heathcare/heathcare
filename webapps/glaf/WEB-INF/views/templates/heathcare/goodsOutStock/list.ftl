...
<script type="text/javascript">
    ...
    function getLink(){
	    var link_ = "${contextPath}/heathcare/goodsOutStock/json?startTime=${startTime}&endTime=${endTime}";
		//alert(link_);
	    return link_;
	}
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				columns:[[
					...
					{title:'出库日期', field:'outStockTime', width:100, align:'center', sortable:true},
   ...
   function formatterKey(value, row, index) {
	...
	function addNew(){
	    var link="${contextPath}/heathcare/goodsOutStock/edit";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/heathcare/goodsOutStock/edit?audit=${audit}&id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
   ...
   function editRow(id){
	    var link = '${contextPath}/heathcare/goodsOutStock/edit?id='+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function auditRow(id){
	    var link = '${contextPath}/heathcare/goodsOutStock/edit?audit=true&id='+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsOutStock/delete?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function onRowClick(rowIndex, row){
	    var link = '${contextPath}/heathcare/goodsOutStock/edit?id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function searchWin(){
	    jQuery('#dlg').dialog('open').dialog('setTitle','物品出库单查询');
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/heathcare/goodsOutStock/edit?id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function auditSelected(){
	    ...
	    var selected = jQuery('#mydatagrid').datagrid('getSelected');
	    if (selected ){
		  var link = '${contextPath}/heathcare/goodsOutStock/edit?audit=true&id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    var link='${contextPath}/heathcare/goodsOutStock/edit?readonly=true&id='+selected.id;
		    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsOutStock/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function searchData(){
        ...
        jQuery.ajax({
                    type: "POST",
                    url: '${contextPath}/heathcare/goodsOutStock/json',
                    dataType:  'json',
                    data: params,
                    error: function(data){
                              alert('服务器处理错误！');
	...
	function batchEdit(){
        var link = '${contextPath}/heathcare/goodsOutStock/batchEdit';
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function auditBatch(){
	  ...
	  if(confirm("确定批量通过审核吗？")){
		if(confirm("审核通过后的数据不能再修改或删除，确定吗？")){
		  jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsOutStock/audit?ids='+objectIds,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function copyPurchase(){
		var link="${contextPath}/heathcare/goodsOutStock/showCopy";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function copyInStock(){
		var link="${contextPath}/heathcare/goodsOutStock/showCopy2";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
    ...
    function doExport() {
        ...
        var link = '${contextPath}/heathcare/reportMain/exportXls?startTime='+startTime+'&endTime='+endTime+'&reportId=GoodsOutStock&ts=${ts}';
		window.open(link);	
	}
	...
	function doWeeklyExport(){
		...
		var link="${contextPath}/heathcare/reportMain/exportXls?reportId=WeeklyGoodsOutStock&ts=${ts}";
		if(startTime != ""){
    ...
    function saveAll(){
	   if(confirm("确定保存数据吗？")){
          ...
          jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/goodsOutStock/updateAll',
				   data: params,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
      ...
      <table valign="middle">
       <tr>
	    <td valign="middle" width="70%">
			...
			<a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-copy'" 
		       onclick="javascript:copyInStock();">复制入库单</a>
		...
		<td valign="middle">
		  ...
		  &nbsp;
		  <a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon_export_xls'"
	         onclick="javascript:doWeeklyExport();">一周出库单</a>
		</td>
       </tr>
      </table>
	</form>
