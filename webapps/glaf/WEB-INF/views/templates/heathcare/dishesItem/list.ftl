...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/heathcare/dishesItem/json?dishesId=${dishesId}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
				        {title:'序号', field:'startIndex', width:60, sortable:false},
						{title:'食物名称',field:'name', width:180, align:"left", formatter:formatterName},
						{title:'用量(克)', field:'quantity', width:60, align:"right"},
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/heathcare/dishesItem/edit?dishesId=${dishesId}&id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function addNew(){
	    var link="${contextPath}/heathcare/dishesItem/edit?dishesId=${dishesId}";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
   ...
   function editRow(id){
	    var link = '${contextPath}/heathcare/dishesItem/edit?dishesId=${dishesId}&id='+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/dishesItem/delete?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/heathcare/dishesItem/edit?dishesId=${dishesId}&id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/heathcare/dishesItem/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
    ...
    function updateName(){
	    ...
	    var link = "${contextPath}/heathcare/dishes/updateDishesName";
