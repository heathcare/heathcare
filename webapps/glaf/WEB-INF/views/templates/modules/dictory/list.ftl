...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'${contextPath}/mydictory/json?nodeId=',
				remoteSort: false,
				singleSelect:true,
				idField:'id',
				columns:[[
					...
					{title:'属性值',field:'value', width:180},
	...
	function addNew(){
		...
		var link = "${contextPath}/mydictory/prepareAdd?nodeId="+nodeId;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onRowClick(rowIndex, row){
		var nodeId = jQuery("#nodeId").val();
	    var link = '${contextPath}/mydictory/prepareModify?id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = "${contextPath}/mydictory/prepareModify?id="+selected.id;
		  jQuery.layer({
				type: 2,
				maxmin: true,
				shadeClose: true,
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    location.href="${contextPath}/mydictory/prepareModify?id="+selected.id;
			jQuery.layer({
				type: 2,
				maxmin: true,
				shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/mydictory/delete?rowIds='+rowIds,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function sortDict(){
	    var nodeId = jQuery("#nodeId").val();
		var link = "${contextPath}/mydictory/showSort?parentId="+nodeId;
		var width=680;
		var height=430;
		var scroll="yes";
		jQuery.layer({
