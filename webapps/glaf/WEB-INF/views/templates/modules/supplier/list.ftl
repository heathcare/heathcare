...
<script type="text/javascript">
   ...
   jQuery(function(){
	...
	function addNew(){
	    var link="${contextPath}/supplier/edit";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/supplier/edit?supplierId='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
    ...
    function editRow(id){
	    var link = '${contextPath}/supplier/edit?supplierId='+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/supplier/delete?supplierId='+id,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/supplier/delete?ids='+str,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
