...
<script type="text/javascript">
   ...
   var setting = {
			async: {
				enable: true,
				url:"${contextPath}/mytreenode/treeJson?parentId=${parentId}",
				dataFilter: filter
			},
			callback: {
    ...
    function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		var link = "${contextPath}/mytreenode/json?parentId="+treeNode.id;
		loadGridData(link);
 	}
    ...
    function getLink(){
	    var link_ = "${contextPath}/mytreenode/json?";
    ...
    jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				//url: '${contextPath}/mytreenode/json?parentId=${parentId}',
				url: getLink(),
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
	...
	function addNew(){
	    //var link="${contextPath}/mytreenode/edit?fromUrl=${fromUrl}";
	    //art.dialog.open(link, { height: 420, width: 680, title: "添加记录", lock: true, scrollbars:"no" }, false);
		//location.href=link;
		var nodeId = jQuery("#nodeId").val();
		link="${contextPath}/mytreenode/edit?parentId="+nodeId;
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onRowClick(rowIndex, row){
	    var link = '${contextPath}/mytreenode/edit?id='+row.id;
		//location.href=link;
		jQuery.layer({
			type: 2,
			maxmin: true,
	...
	function editRow(id){
		link="${contextPath}/mytreenode/edit?id="+id;
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("删除数据不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/mytreenode/deleteById?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/mytreenode/edit?id='+selected.id+'&category=${category}';
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    location.href='${contextPath}/mytreenode/edit?id='+selected.id;
		}
	}
	...
	function searchData(){
        ...
        jQuery.ajax({
                    type: "POST",
                    url: '${contextPath}/mytreenode/json',
                    dataType:  'json',
                    data: params,
                    error: function(data){
                              alert('服务器处理错误！');
    ...
    function sortItems(){
	  var nodeId = jQuery("#nodeId").val();
	  var link = "${contextPath}/mytreenode/showSort?parentId="+nodeId;
	  var width=680;
	  var height=430;
	  var scroll="yes";
	  jQuery.layer({
...
<div class="easyui-layout" data-options="fit:true">  
   ...
   <div data-options="region:'center'"> 
	<div class="easyui-layout" data-options="fit:true">  
	  <div data-options="region:'center',border:false" class="toolbar-backgroud" style="height:40px;">
