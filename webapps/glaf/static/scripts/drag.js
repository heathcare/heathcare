...
glaf.drag = (function () {
	...
	handler_onmousedown = function (e) {
		...
		glaf.drag.obj_old = obj_old = obj || this;
		// set reference to the clicked object
		glaf.drag.obj = obj = this;
		...
		if(tables[table_source] == undefined) {return false;}
		glaf.drag.source_cell = source_cell = tables[table_source].rows[row_source].cells[cell_source];
		glaf.drag.current_cell = current_cell = source_cell;
		glaf.drag.previous_cell = previous_cell = source_cell;
		...
		if (mouseButton === 1) {
			...
			glaf.drag.myhandler_clicked(); // call myhandler (public method)
	...
	handler_onmouseup = function (e) {
		...
		// if ordinary element was clicked and left button was released, but element is placed inside unmovable table cell
			...
			glaf.drag.myhandler_notmoved();
		}		
		else {
			// if current table is in range, use table for current location
			if (table < tables.length) {
				target_table = tables[table];
				glaf.drag.target_cell = target_cell = target_table.rows[row].cells[cell];
			}
			// if any level of old position is undefined, then use source location
				...
				glaf.drag.target_cell = target_cell = target_table.rows[row_source].cells[cell_source];
			}
			// or use the previous location
			else {
				target_table = tables[table_old];
				glaf.drag.target_cell = target_cell = target_table.rows[row_old].cells[cell_old];
			}
			...
			else if (cloned_flag === 1 && glaf.drag.delete_cloned === true &&
			...
			// remove object if destination cell has "trash" in class name
			else if (target_cell.className.indexOf(glaf.drag.trash) > -1) {
			...
			else if (glaf.drag.drop_option === 'switch') {
				...
				if (target_elements_length) {
					...
					glaf.drag.myhandler_switched();
					// and myhandler_dropped
					glaf.drag.myhandler_dropped(target_cell);
					// if object is cloned, update climit1_X or climit2_X classname
					if (cloned_flag === 1) {
						clone_limit();
					}
			...
			else if (glaf.drag.drop_option === 'overwrite') {
			...
			if (table_source !== null && row_source !== null) {
				...
				tables[table_old].rows[row_old].cells[cell_old].style.backgroundColor = '#f6f6f6';
			}
	...
	handler_onmousemove = function (e) {
		var evt = e || window.event,	// define event (FF & IE)
			bound = glaf.drag.bound,	// read "bound" public property (maybe code will be faster, and it will be easier to reference in onmousemove handler)
		...
		if (moved_flag === 0 && (obj.className.indexOf('clone') > -1 || (glaf.drag.clone_ctrlKey === true && evt.ctrlKey))) {
			clone_object();
			cloned_flag = 1;
			glaf.drag.myhandler_cloned();
		...
		if (evt.clientX < div_box[1] &&	evt.clientX > div_box[3] &&
			...
			if (table < tables.length && (table !== table_old || row !== row_old || cell !== cell_old)) {
				...
				if (table_old !== null && row_old !== null && cell_old !== null) {
					...
					glaf.drag.previous_cell = previous_cell = tables[table_old].rows[row_old].cells[cell_old];
					// define current table cell
					glaf.drag.current_cell = current_cell = tables[table].rows[row].cells[cell];
					// if drop option is 'switching' then replace content from current cell to the previous cell
					if (glaf.drag.drop_option === 'switching') {
						move_object(current_cell, previous_cell);
					}
					// target cell changed - call public handler
					glaf.drag.myhandler_changed();
	...
	set_trc = function (evt) {
		...
		for (table = 0; table < tables.length; table++) {
			...
			if (tables[table].offset[3] < evt.clientX  &&  evt.clientX < tables[table].offset[1] &&	
				...
				if (cell_current.className.indexOf(glaf.drag.trash) === -1) {
					// search for 'only' class name
					only_found = cell_current.className.indexOf(glaf.drag.only.cname) > -1 ? true : false;
					...
					// DIV objects marked with "only" can't be placed to other cells (if property "other" is "deny")
						...
						mark_found = cell_current.className.indexOf(glaf.drag.mark.cname) > -1 ? true : false;
						...
						if ((mark_found === true && glaf.drag.mark.action === 'deny') || (mark_found === false && glaf.drag.mark.action === 'allow')) {
				...
				if ((glaf.drag.drop_option === 'single' || single_cell) && cell_current.childNodes.length > 0) {
	...
	set_color = function () {
		...
		if (table < tables.length && table !== null && row !== null && cell !== null) {
			...
			tables[table].rows[row].cells[cell].style.backgroundColor = glaf.drag.hover_color;
	...
	autoscrollX = function (so) {
		...
		if (edge.flag.x > 0 && ((edgeCrossed < 0 && scrollPosition > 0) || (edgeCrossed > 0 && scrollPosition < maxsp))) {
			...
			setTimeout(glaf.drag.autoscrollX, glaf.drag.speed);
		}
	...
	autoscrollY = function (so) {
		...
		if (edge.flag.y > 0 && ((edgeCrossed < 0 && scrollPosition > 0) || (edgeCrossed > 0 && scrollPosition < maxsp))) {
			...
			// recursive autoscroll call 
			setTimeout(glaf.drag.autoscrollY, glaf.drag.speed);
		}
	...
	clone_object = function () {
		...
		glaf.drag.obj_old = obj_old = obj;
		// set reference to the cloned object	
		glaf.drag.obj = obj = obj_new;
	};
		...
		if (match_arr !== null) {
			...
			if (limit <= 0) {
				...
				if (limit_type === 2) {
					...
					glaf.drag.myhandler_clonedend2();		// call myhandler_clonedend2 handler 
				}
				else {
					// call myhandler_clonedend1 handler
					glaf.drag.myhandler_clonedend1();
				}
			}
	...
	enable_drag = function (enable_flag, div_id) {
		...
		if (enable_flag === true || enable_flag === 'init') {
			...
			borderStyle = glaf.drag.border;
			cursor = 'move';
			enabled = true;
		}
		else {
			handler = null;
			borderStyle = glaf.drag.border_disabled;
			cursor = 'auto';
			enabled = false;
		}
	...
	return {
		...
		/*
		 * Action handlers
		 * Each handler sees glaf.drag.obj, glaf.drag.obj_old, glaf.drag.target_cell ... reference
		 * Note: for the first dragging, glaf.drag.obj_old === glaf.drag.obj because glaf.drag.obj_old does not exist yet
		 */
